...
# В данном скрипте производятся несколько разных операций, сумма которых будет равна:
# T(n) = 10(for) + 5(const) + 1(print) + 10(for) + 2(print) + 1(>) + 2(const) + от 0 до 8(for) + 1(print) = от 32 до 40
...
# Использование встроенных функций дало незначительный прирост скорости исполнения скрипта,
# в сравнение с исходным вариантом.
